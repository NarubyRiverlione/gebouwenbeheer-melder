...
const fetchFn = (globalThis as any).fetch as (input: RequestInfo, init?: RequestInit) => Promise<Response>
...
async function main() {
  try {
    const raw = await fs.readFile(new URL("./ExampleReports.json", import.meta.url), "utf-8")
    ...
    for (const [i, report] of reports.entries()) {
      try {
        const res = await fetchFn(REPORTS_URL, {
          ...
          body: JSON.stringify({ report, processNow: true }),
        })
        if (!res.ok) {
